...
void VoronoiSaverNode::mapCallback(const grid_map_msgs::GridMap& msg)
{
  ...
  ROS_INFO("Saved map (%s):  size: [%i, %i]; resolution: %f; origin: [%f, %f]", mapInfoName_.c_str(), info.cols,
           info.rows, info.resolution, info.origin_cols, info.origin_rows);

  got_map_ = true;
}

VoronoiSaverNode::VoronoiSaverNode(ros::NodeHandle& n) : voronoi_map::VoronoiSerializer(), n_(n), n_param_("~")
{
