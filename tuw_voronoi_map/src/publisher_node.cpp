...
VoronoiPublisherNode::VoronoiPublisherNode(ros::NodeHandle& n) : voronoi_map::VoronoiSerializer(), n_(n), n_param_("~")
{
  topicVoronoiMap_ = "voronoi_map";
  n_param_.getParam("map_topic", topicVoronoiMap_);

  mapInfoName_ = "vmap.info";
  n_param_.getParam("mapInfoName", mapInfoName_);

  mapPath_ = "./";
  n_param_.getParam("mapPath", mapPath_);

  loadMap(mapInfoName_, mapPath_);

  VoronoiMapInfo info = getMapInfo();
  ROS_INFO("Publishing map (%s):  size: [%i, %i]; resolution: %f; origin: [%f, %f]", mapInfoName_.c_str(), info.cols,
           info.rows, info.resolution, info.origin_cols, info.origin_rows);
