...
class VoronoiExpansion : public Expander {
    private:
	...
	voronoi_map::VoronoiMapProvider *v_map_;
	...
	**/
	 ...
	 **/
	void getMaps(float *distance_field, int8_t *voronoi_graph, int8_t *global_map, grid_map::GridMap *voronoi_map);
	...
	* @param v_map	the voronoi map which updates automatically over ros
	**/
	VoronoiExpansion(PotentialCalculator* p_calc, int xs, int ys, Heuristics *hx, voronoi_map::VoronoiMapProvider *v_map);
