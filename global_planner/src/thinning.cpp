...
void greyscale_thinning(cv::Mat &src, cv::Mat &dst)
{
  ...
  for (int i = 1; i < dst.rows - 1; i++)
  {
    for (int j = 1; j < dst.cols - 1; j++)
    {
      ...
      {
        // Find neighbour size
        float p[8] = { src.at<float_t>(i, j - 1),     src.at<float_t>(i - 1, j - 1), src.at<float_t>(i - 1, j),
                       src.at<float_t>(i - 1, j + 1), src.at<float_t>(i, j + 1),     src.at<float_t>(i + 1, j + 1),
                       src.at<float_t>(i + 1, j),     src.at<float_t>(i + 1, j - 1) };

        bool bigger[8] = { p[0] > pAct, p[1] > pAct, p[2] > pAct, p[3] > pAct,
                           p[4] > pAct, p[5] > pAct, p[6] > pAct, p[7] > pAct };
        bool smaller[8] = { p[0] < pAct, p[1] < pAct, p[2] < pAct, p[3] < pAct,
                            p[4] < pAct, p[5] < pAct, p[6] < pAct, p[7] < pAct };
        ...
        // Maxima
        else if (!bigger[0] && !bigger[1] && !bigger[2] && !bigger[3] && !bigger[4] && !bigger[5] && !bigger[6] &&
                 !bigger[7])
        {
          dst.at<int8_t>(i, j) = initCosts;
          q.push(Index(i, j, pAct));
        ...
        if (pAct > 1)  // Dont look for duplex saddle points at the edges of the map
        {
          ...
          if ((!smaller[5] || !smaller[6] || !smaller[4]) && !bigger[7] && !bigger[3])
          {
            ...
            float pp2[8] = { src.at<float_t>(m, n - 1), src.at<float_t>(m - 1, n - 1),
                             src.at<float_t>(m - 1, n), src.at<float_t>(m - 1, n + 1),
                             src.at<float_t>(m, n + 1), src.at<float_t>(m + 1, n + 1),
                             src.at<float_t>(m + 1, n), src.at<float_t>(m + 1, n - 1) };
            bool bigger2[8] = { pp2[0] > pot, pp2[1] > pot, pp2[2] > pot, pp2[3] > pot,
                                pp2[4] > pot, pp2[5] > pot, pp2[6] > pot, pp2[7] > pot };
            bool smaller2[8] = { pp2[0] < pot, pp2[1] < pot, pp2[2] < pot, pp2[3] < pot,
                                 pp2[4] < pot, pp2[5] < pot, pp2[6] < pot, pp2[7] < pot };
...
Index getMaximumNeighbour(int i, int j, const cv::Mat &src, cv::Mat &dst)
{
  ...
  Index p[8] = { Index(i, j - 1, src.at<float_t>(i, j - 1)), Index(i - 1, j - 1, src.at<float_t>(i - 1, j - 1)),
                 Index(i - 1, j, src.at<float_t>(i - 1, j)), Index(i - 1, j + 1, src.at<float_t>(i - 1, j + 1)),
                 Index(i, j + 1, src.at<float_t>(i, j + 1)), Index(i + 1, j + 1, src.at<float_t>(i + 1, j + 1)),
                 Index(i + 1, j, src.at<float_t>(i + 1, j)), Index(i + 1, j - 1, src.at<float_t>(i + 1, j - 1)) };
